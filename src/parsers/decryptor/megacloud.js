...
import { getIndoSubtitle } from "../../extractor/subtitle.helper.js";
...
export async function megacloud({ selectedServer, id }) {
  ...
  try {
    ...
    let tracks = rawSourceData.tracks ?? [];

    const slug = id.split("::")[0]; // sesuaikan slug kamu

    const indoUrl = await getIndoSubtitle(slug);
